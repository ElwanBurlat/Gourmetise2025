<template>
  <v-snackbar v-model="snackbarGreen" color="success" timeout="2500" location="top">
  ...
  <v-container class="v-container">
    <v-row>
      <v-col>
        ...
        <v-text-field v-model="siret" label="Siret" />
        <v-text-field v-model="companyName" label="Nom de la boulangerie" />
        <v-text-field v-model="phone" label="Telephone" />
        <v-text-field v-model="adress" label="Adresse" />
        <v-text-field v-model="postalcode" label="Code Postal" @input="findCity" />
        <v-select v-model="city" :items="cities" label="Ville" v-if="cities.length > 1" />

        <v-text-field v-model="city" label="Ville" readonly v-else />

        <v-text-field v-model="nameContact" label="Nom du Contact" />
        <v-text-field v-model="phoneContact" label="Telephone du contact" />
...
const fetchPosts = async () => {
  ...
  try {
    const response = await axios.post('http://localhost:8000/api/bakery', {
      siret: siret.value,
...
const findCity = async () => {
  const response = await axios.get(
    `https://geo.api.gouv.fr/communes?codePostal=${postalcode.value}`
  );
